const drawInvestmentPanel = (function () {
    ...
    function updateProbabilisticValues(driver) {
        if (investmentState.workingDays.length === 0) {
            initializeDefaultWorkingDays(new Date().getFullYear());
        }
    ...
    function calculateFinancialScenario(annualUnitDemand) {
        ...
        const avgPrice = (finInputs.superSell * ratios.super) + (finInputs.ultraSell * ratios.ultra) + (finInputs.megaSell * ratios.mega);
        ...
        if (!runExpansionCase) {
            ...
            equipmentCostForDepreciation = (costPerFootStraight * ASSEMBLY_LINE_LENGTH) + (costPerBend * ((4 * currentEmployees) - (currentEmployees % 2 === 0 ? 2 : 0))) + installationCost;
            ...
            const newLineCost = (costPerFootStraight * ASSEMBLY_LINE_LENGTH) + (costPerBend * ((4 * optimalConfig.empCount) - (optimalConfig.empCount % 2 === 0 ? 2 : 0)));
        ...
        for (let t = 1; t <= analysisPeriod; t++) {
            ...
            const avgMatCost = (finInputs.superCogs * ratios.super) + (finInputs.ultraCogs * ratios.ultra) + (finInputs.megaCogs * ratios.mega);
            const avgReworkCost = (finInputs.superRework * ratios.super) + (finInputs.ultraRework * ratios.ultra) + (finInputs.megaRework * ratios.mega);
    ...
    function runFullAnalysis() {
        ...
        // Only update internal tab UI if active
    ...
    function renderInvestmentResults(results) {
        ...
        if (chartSvg.empty()) {
            ...
            chartG = chartSvg.append("g").attr("transform", `translate(${margin.left},${margin.top})`);
            chartG.append("g").attr("class", "inv-axis x-axis");
            chartG.append("g").attr("class", "inv-axis y-axis");
            chartSvg.append("text").attr("class", "inv-axis-label label-x").attr("text-anchor", "middle");
            chartSvg.append("text").attr("class", "inv-axis-label label-y").attr("transform", "rotate(-90)").attr("text-anchor", "middle");
            chartG.append("line").attr("class", "inv-break-even");
        } else {
            chartG = chartSvg.select("g");
        }
        ...
        const areas = chartG.selectAll(".inv-area").data([
            { data: p90Data, y0: p50Data, fill: getComputedStyle(root).getPropertyValue('--primary') },
            { data: p50Data, y0: p10Data, fill: getComputedStyle(root).getPropertyValue('--secondary2') }
        ]);
        ...
        const colorScale = d3.scaleOrdinal().domain(['P90 (Optimistic)', 'P50 (Most Likely)', 'P10 (Conservative)']).range([getComputedStyle(root).getPropertyValue('--primary'), getComputedStyle(root).getPropertyValue('--secondary1'), getComputedStyle(root).getPropertyValue('--secondary2')]);
        ...
        chartG.selectAll(".inv-hitbox").on("mouseover", (e, d) => {
        ...
        }).on("mousemove", (e) => chartTooltip.style("left", (e.pageX + 15) + "px").style("top", (e.pageY - 28) + "px")).on("mouseout", () => chartTooltip.transition().duration(500).style("opacity", 0));
    }
